...
let isOwner = false;
let socket = null;
let currentMembers = [];if (!roomId) window.location.href = '/';// ============ 요소 ============
const roomTitleEl = document.getElementById('roomTitle');
const memberCountTag = document.getElementById('memberCountTag');
const messageArea = document.getElementById('messageArea');
const messageInput = document.getElementById('messageInput');
const sendBtn = document.getElementById('sendBtn');
const leaveRoomBtn = document.getElementById('leaveRoomBtn');const noticeBar = document.getElementById('noticeBar');
...
const noticeSaveBtn = document.getElementById('noticeSaveBtn');const plusBtn = document.getElementById('plusBtn');
const plusMenu = document.getElementById('plusMenu');
const menuImageBtn = document.getElementById('menuImageBtn');
const menuPollBtn = document.getElementById('menuPollBtn');
const fileInput = document.getElementById('fileInput');const pollModal = document.getElementById('pollModal');
...
const pollError = document.getElementById('pollError');const memberListOpenBtn = document.getElementById('memberListOpenBtn');
...
const memberListBody = document.getElementById('memberListBody');const kickConfirmModal = document.getElementById('kickConfirmModal');
...
let pendingKickSocketId = null;const mediaViewer = document.getElementById('mediaViewer');
const mediaViewerContent = document.getElementById('mediaViewerContent');
const mediaViewerCloseBtn = document.getElementById('mediaViewerCloseBtn');// ============ 초기화 ============
init();async function init() {
  ...
  myProfile = profileData.profile;  const uidRes = await fetch('/api/my-uid');
  const uidData = await uidRes.json();
  if (uidData.success) myUid = uidData.uid;  const roomRes = await fetch(/api/rooms/${roomId});
  ...
  updateNotice(roomData.room.notice);  await loadPreviousMessages();
  connectSocket();
...
menuPollBtn.addEventListener('click', () => {
  ...
  pollOptionsWrap.innerHTML =     <input type="text" class="poll-option-input" placeholder="항목 1" maxlength="50">     <input type="text" class="poll-option-input" placeholder="항목 2" maxlength="50">  ;
  pollError.textContent = '';
  pollModal.classList.remove('hidden');
});
pollCancelBtn.addEventListener('click', () => pollModal.classList.add('hidden'));addPollOptionBtn.addEventListener('click', () => {
...
function renderMessage(msg) {
  if (msg.type === 'system') {
    ...
    return;
  }  const mine = isMineMessage(msg);  const row = document.createElement('div');
  row.className = 'msg-row' + (mine ? ' mine' : '');
  row.dataset.messageId = msg._id;  const avatarDiv = document.createElement('div');
  ...
  body.className = 'msg-body';  const nicknameDiv = document.createElement('div');
  nicknameDiv.className = 'msg-nickname';
  nicknameDiv.textContent = msg.senderNickname || '';  let bubble;
  if (msg.type === 'text') {
  ...
  timeDiv.textContent = formatTime(msg.createdAt);  if (!mine) body.appendChild(nicknameDiv);
  body.appendChild(bubble);
  body.appendChild(timeDiv);  row.appendChild(avatarDiv);
...
function buildPollCard(msg) {
  ...
  const totalVotes = msg.poll.options.reduce((sum, o) => sum + o.count, 0);  card.innerHTML = <div class="poll-question">📊 ${escapeHtml(msg.poll.question)}</div>;
  msg.poll.options.forEach((opt, idx) => {
    ...
    optDiv.innerHTML =       <div class="poll-option-fill" style="width:${pct}%"></div>       <div class="poll-option-content">         <span>${escapeHtml(opt.text)}</span>         <span>${opt.count}표</span>       </div>    ;
...
memberPanelOverlay.addEventListener('click', closeMemberPanel);function renderMemberList() {

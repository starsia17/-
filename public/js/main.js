...
const profileError = document.getElementById('profileError');const myAvatar = document.getElementById('myAvatar');
const myAvatarFallback = document.getElementById('myAvatarFallback');
const myNickname = document.getElementById('myNickname');const roomList = document.getElementById('roomList');
const emptyRooms = document.getElementById('emptyRooms');
const pagination = document.getElementById('pagination');const createRoomOpenBtn = document.getElementById('createRoomOpenBtn');
...
const createRoomError = document.getElementById('createRoomError');const passwordModal = document.getElementById('passwordModal');
const passwordRoomTitle = document.getElementById('passwordRoomTitle');
const passwordInput = document.getElementById('passwordInput');
const passwordCancelBtn = document.getElementById('passwordCancelBtn');
const passwordSubmitBtn = document.getElementById('passwordSubmitBtn');
const passwordError = document.getElementById('passwordError');let selectedAvatarBase64 = '';// ============ 초기화 ============
...
document.querySelector('.avatar-picker').addEventListener('click', () => avatarInput.click());avatarInput.addEventListener('change', () => {
...
async function loadRooms(page) {
  ...
  const data = await res.json();
  if (!data.success) return;  roomList.innerHTML = '';
  if (data.rooms.length === 0) {
    ...
    data.rooms.forEach(room => {
      ...
      card.innerHTML =         <div class="room-card-info">           <div class="room-card-title">${escapeHtml(room.title)} ${room.hasPassword ? '<span class="lock-badge">🔒</span>' : ''}</div>           <div class="room-card-meta">만든이 ${escapeHtml(room.creatorNickname)} · ${room.memberCount}명 참여중</div>         </div>         <button class="room-card-enter">입장</button>      ;
...
createRoomCancelBtn.addEventListener('click', () => createRoomModal.classList.add('hidden'));createRoomSubmitBtn.addEventListener('click', async () => {
